...
public class Graph<T> {
    ...
    public void initStatuses() {
        for (int i = 0; i < N; i++) {
            statuses[i] = Status.UN_DISCOVERD;
        }
    }
    ...
    public void DijkstraPath(int index) {
        ...
        while (!isAllVisited()) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                // 如果节点未访问, 且是邻接节点
                if (statuses[i] == Status.UN_DISCOVERD && matrix[from][i] > 0
    ...
    private int indexOfMin(int[] nums) {
        ...
        for (int i = 0; i < N; i++) {
            if (statuses[i] == Status.UN_DISCOVERD) {
                remain.add(i);
            }
        }
        if (remain.size() == 0) {
